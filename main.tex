...
Suppose a robot $A$ is a rigid-body moving in some world space $W$. Let $Q$ be the Configuration-Time Space of $A$ as defined in the above section. Let $Q_{obs}$ be the obstacle region in $Q$. The free region is defined as $Q_{free} = Q \backslash Q_{obs}$.

Given an initial state $q_I \in Q_{free}$ and a goal state $q_G \in Q_{free}$, a \emph{complete} algorithm will find a path $\rho[0:1]->Q_{free}$, such that $\rho[0]=q_I$ and $\rho[1]=q_G$.


Once $\rho$ has been found, each state is treated as a knot point on a trajectory $T$ and intermediate points along $T$ are interpolated to form a full trajectory.
...
Kinodynamic Planning is an extension of the general motion planning problem that integrates the kinematic and dynamic constraints into the final solution. Prior to the this field, robot motion was separated into two problems: path-planning and trajectory-following. The approaches described so far in this paper have fit into this decoupled model (the sampling-based approaches can be extended to kinodynamic planning).

The problem was first proposed in \cite{donald1993kinodynamic}, where an approximate solution was presented. This work focused on a graph-search approach on the phase space of a robot's C-space. 
